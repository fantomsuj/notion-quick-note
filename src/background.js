...
const refreshStoredToken = createAccessTokenRefresher({
...
chrome.runtime.onStartup.addListener(() => {
  void initializeWorker();
  void provisionDatabaseWhenReady();
});
...
async function handleMessage(message, sender) {
  ...
  switch (message.type) {
    ...
    case "OPEN_SETTINGS":
      ...
      return { ok: true };
    default:
      return { ok: false, error: "Unsupported Quick Note message." };
  }
}
...
async function deliverRecord(record, connection) {
  ...
  while (true) {
    try {
      ...
      if (isUnauthorized(error) && connection.refreshToken && !refreshedToken) {
        let refreshed;
        try {
          refreshed = await refreshStoredToken({ ...connection.settings, token });
        } catch {
          throw new NotionApiError("Reconnect Notion to deliver this capture.", { status: 401, code: "refresh_failed" });
        }
...
async function findExistingRecord(record, connection) {
  ...
  try {
    ...
    try {
      ...
      throw new NotionApiError("Reconnect Notion to verify this capture.", { status: 401, code: "refresh_failed" });
    }
...
async function ensureDefaultDatabase() {
  try {
    ...
    const settings = await getSettings();
    if (!isUnauthorized(error) || !settings.refreshToken) throw error;
...
async function validateDestinationSelection(destination = {}) {
  ...
  try {
    ...
    if (isUnauthorized(error) && settings.refreshToken) {
      try {
...
async function disconnectNotion(confirmed) {
  ...
  if (settings.authType === "oauth" && settings.token && brokerUrl) {
    try {
      await revokeAccessToken({ brokerUrl, token: settings.token });
    } catch {
      warning = [warning, "Disconnected locally, but Notion could not confirm token revocation."].filter(Boolean).join(" ");
    }
  }
  await chrome.storage.local.remove([
    "token", "refreshToken", "workspaceId", "workspaceName", "workspaceIcon", "botId", "connectionId",
