import { exchangeAuthorizationCode } from "../src/oauth.js";
...
async function connectWithToken() {
  ...
  await storeConnection({ authType: "token", token, refreshToken: "" });
  $("#token").value = "";
  await provisionDefaultDatabase();
}
...
async function connectOAuth() {
  ...
  setBusy($("#oauth-connect"), true, "Connecting…");
  try {
    ...
    const payload = await exchangeAuthorizationCode({ brokerUrl, code, redirectUri });
    ...
    setBusy($("#oauth-connect"), false, "Connect Notion");
  }
}
...
async function storeConnection(connection) {
  ...
  settings = { ...settings, ...connection, ...resetDestination };
  await chrome.storage.local.set({ ...connection, ...resetDestination });
